...
import axiosInstance from "@/apis/axiosConfig"; // Adjust this path if needed
...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // The API endpoint should match your backend router setup.
      // Your user.router.ts was mounted on '/users', so the endpoint is likely '/users/login'
      ...
      if (response.data && response.data.result) {
        ...
        // --- KEY CHANGE: Redirect based on numeric role ---
        ...
        switch (userRole) {
          ...
          case 1: // Candidate
            ...
            break;
          case 0: // Admin
            navigate('/admin/dashboard'); // Or another admin-specific route
            break;
          default:
            // Fallback for any unknown role
            console.error("Unknown user role received:", userRole);
            navigate('/'); // Redirect to home page as a safe default
            break;
        }
          ...
          <CardContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center space-x-2 text-sm">
                  <input type="checkbox" className="rounded border-gray-300 text-orange-600 focus:ring-orange-500" disabled={loading} />
                  <span className="text-gray-600">Remember me</span>
                </label>
                <Link to="/forgot-password" className={`text-sm text-orange-600 hover:text-orange-700 hover:underline ${loading ? 'pointer-events-none opacity-50' : ''}`}>
                  Forgot password?
                </Link>
              </div>
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-600">
                ...
                <Link to="/register" className={`text-orange-600 hover:text-orange-700 font-semibold hover:underline ${loading ? 'pointer-events-none opacity-50' : ''}`}>
                  Sign up here
                </Link>
              </p>
            </div>
